...
export class GovernmentsIDPageUK extends WebActionsPage {
  ...
  constructor(page: Page, givenname: string, FamilyName: string, context: BrowserContext) {
    ...
    this.idChange = page.locator('//div[@data-automation-id="titleText" and text()="Change/Update My Government IDs"]');
    ...
    this.contractAddendumtext = page.locator('h3:has-text("Czechia Contract Addendum Info")');
  ...
  async AddID(): Promise<void> {
    ...
    await this.GCountry.selectOption({ label: 'Czechia' });
    ...
    await this.IssuedBy.fill('Czechia');
