...
const excelFileName = 'JobChange/Workday_JobChange_Romania.xlsx';
    ...
    dataSet.forEach((data, index) => {
      ...
      test(`@JobChange - Test ${index + 1} `, async ({ page, context, login, hireEmployee, appCommon, proxy }) => {
        try {
          ...
          await hrInbxPage.setContractAddendumInfoIfVisible(
            data.ContractAddendumInfo_TerminateAddendumEffectiveDate,
            data.ContractAddendumInfo_AddendumCreationDate,
            data.ContractAddendumInfo_AddendumEndDate);

          await capObj.checkForScreenErrors()
          await appCommon.SuccessEventHandle();
          if (data.Process.toString().includes("Promotion_Manager")) {
            await hrInbxPage.clickInboxMyTaskAndSubmitIfVisible("Edit Notice Periods for Promotion:");
            await capObj.checkForScreenErrors()
            await appCommon.SuccessEventHandle();
          } else {
            await hrInbxPage.clickInboxMyTaskAndSubmitIfVisible("Edit Notice Periods for Data Change:");
            await capObj.checkForScreenErrors()
            await appCommon.SuccessEventHandle();
          ...
          const HRidforProposeCompensation = await appCommon.getJobChangesHRpartnerID(givenName, familyName);

          if (HRidforProposeCompensation !== HRPartner) {
            await appCommon.Searchbox("Start Proxy");
            await proxy.startProxy(HRidforProposeCompensation);
          }
          await appCommon.MyTasks();
          await proposeCompensation.setProposeCompensationHire(data.GradeProfile, data.Step, data.ProposeCompensationChange_Salary, "NaN", "NaN");
          await capObj.checkForScreenErrors();
