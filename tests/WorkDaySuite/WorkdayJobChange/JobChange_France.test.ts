...
const excelFileName = 'JobChange/Workday_JobChange_France.xlsx';
    ...
    dataSet.forEach((data, index) => {
      ...
      test(`@JobChange - Test ${index + 1} `, async ({ page, context, login, hireEmployee, appCommon, proxy }) => {
        try {
          ...
          await hrInbxPage.clickInboxMyTaskAndSubmitIfVisible("Enter New Probationary Dates: Data Change:");
          await hrInbxPage.clickInboxMyTaskAndSubmitIfVisible("Enter New Probationary Dates: Promotion:");
          await capObj.checkForScreenErrors()
          await appCommon.SuccessEventHandle();
