...
const excelFileName = 'Hires/Workday_NewHire_Belgium_Regression_PK6.xlsx';
...
for (const sheetName in sheetsJson) {
  ...
  dataSet.forEach((data, index) => {
    ...
    if (data.TestStatus != 'Passed') {
      ...
      test(`@HirePK6 Employee - Test ${index + 1} `, async ({ page, appCommon, context, login, home, hireEmployee, proxy }) => {
        try {
