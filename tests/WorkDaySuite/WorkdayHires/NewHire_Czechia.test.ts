      ...
      test(`@HirePK6 Employee - Test ${index + 1} `, async ({ page, context, login, home, hireEmployee, appCommon, proxy }) => {
        try {
          ...
          if (data.JobProfile.toString().includes("Manager")) {// || data.Position.toString() != "No") {
            ...
            position = await createPostition.createPositionForManager(data.HireDate.toString(), data.HireDate.toString(), String(data.EmployeeType).trim(), String((data.JobProfile)).trim(), String(data.TimeType).trim(), data.Location);
