...
const excelFileName = 'Hungary_New Hire_Store 970_automationFileM.xlsx';
    ...
    if (data.TestStatus !== "Passed") {
      ...
      test(`@Hire Employee - Test ${index + 1} `, async ({ page, context, login, home, hireEmployee, appCommon, proxy }) => {
        try {
          ...
          // await empInboxpage.changePersonalInformationHungaryMk(data.Gender, data.DateOfBirth, data.CityOfBirth, data.MaritalStatus,"NaN", data.CitizenshipStatus, data.PrimaryNationality, data.CountryOfBirth, "NaN", "NaN", "NaN");
          // await capObj.checkForScreenErrors();
          await empInboxpage.changePersonalInformationHun(data.Gender, data.DateOfBirth, data.CountryOfBirth, data.CityOfBirth, data.MaritalStatus, data.CitizenshipStatus, data.PrimaryNationality, "NaN");
          await capObj.checkForScreenErrors();
          await appCommon.SuccessEventHandle();
