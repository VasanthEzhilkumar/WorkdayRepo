...
const excelFileName = 'Terminations/Workday_Termination_Poland_Regression_PK14.xlsx';
  ...
  dataSet.forEach((data, index) => {
    ...
    test(`@Termination - Test ${index + 1} `, async ({ page, context, login, appCommon, proxy }) => {
      try {
        ...
        const HRPartner = await appCommon.getSecondTerminationHRpartnerID(givenName, familyName);
