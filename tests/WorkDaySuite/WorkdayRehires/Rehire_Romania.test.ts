...
const excelFileName = 'Rehires/Workday_Rehire_Romania_Regression_PK17.xlsx';
  ...
  dataSet.forEach((data, index) => {
    ...
    test(`@Rehire Employee - Test ${index + 1} `, async ({ page, appCommon, context, login, home, hireEmployee, proxy }) => {
      try {
        ...
        //obtain HR partner ID for hr proxy
        ...
        await hrInbxPage.clickInboxMyTaskAndSubmit("contact information: Hire:");
        await appCommon.SuccessEventHandle();

        await hrInbxPage.clickInboxMyTaskAndSubmit("Bank Details: Hire:");
        await appCommon.SuccessEventHandle();
