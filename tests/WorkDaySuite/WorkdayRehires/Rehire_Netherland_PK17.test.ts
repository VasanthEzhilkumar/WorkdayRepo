...
const excelFileName = 'Rehires/Workday_Rehire_Netherland_Regression_PK17.xlsx';
        ...
        if (data.TestStatus !== "Passed") {
            ...
            test(`@Rehire Employee - Test ${index + 1} `, async ({ page, appCommon, context, login, home, hireEmployee, proxy }) => {
                try {
                    ...
                    await hrInbxPage.clickInboxMyTaskAndSubmit("Payroll Tax Deduction NL:");
                    await appCommon.SuccessEventHandle();

                    await hrInbxPage.clickInboxMyTaskAndSubmit("WW aansturing:");
                    await appCommon.SuccessEventHandle();
                    ...
                    await empInboxpage.clickInboxMyTaskAndSubmit("Add Certifications (External - Netherlands)");
                    await appCommon.SuccessEventHandle();
