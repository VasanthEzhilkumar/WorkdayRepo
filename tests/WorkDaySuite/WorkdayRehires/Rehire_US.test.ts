...
const excelFileName = 'Rehires/Workday_Rehire_USA_Regression_PK14.xlsx';
...
for (const sheetName in sheetsJson) {
  ...
  dataSet.forEach((data, index) => {
    ...
    test(`@Rehire - Test ${index + 1} `, async ({ page, context, login, home, hireEmployee, appCommon, proxy }) => {
      try {
