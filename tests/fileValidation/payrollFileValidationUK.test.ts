  ...
  dataSet.forEach((data, index) => {
    ...
    test(`@Hire Employee - Test ${index + 1} `, async ({ page, context, login, home, hireEmployee, appCommon, proxy, fileValidationUK }) => {
      try {
        ...
        // page.evaluate("document.body.style.zoom=0.50");
        ...
        await fileValidationUK.searchIntegrationTitle();
        await fileValidationUK.clickMoreCategories();
        await fileValidationUK.launchINT030UKSAPOutboundCDPECI();
        await fileValidationUK.setIntegrationCriteria();
