...
const excelFileName = 'Slovenia  795 JTS Automation File -Final.xlsx';
...
test.describe('Test', () => {
    ...
    test.beforeEach('Login - ', async ({ login }) => {
    ...
    for (const sheetName in sheetsJson) {
        ...
        dataSet.forEach((data, index) => {
            ...
            test(`@Hire Employee check - Test ${index + 1} `, async ({ page, context, home, appCommon, proxy }) => {
                try {
                    ...
                    let flag = await empCareerPage.addEmpCertificationCheck(data.Job);
                    if (!flag) {
                        new throws(error);
                    } {
